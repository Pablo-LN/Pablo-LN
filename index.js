...
const preferedPool = 'japan.mibpool.com';

const whitelistAddresses = [
    '0x6aab008a9d1743c3c118d89a49301aa429c4fc87'
];
...
const getApiClient = () => {
    ...
    return {
        getMightyMiners: async (pools) => {
            const miners = await Promise.all(pools.map(async pool => {
                ...
                try {
                    ...
                    return Promise.all(Object.entries(body.miners)
                        .filter(e => {
                            if (pool === preferedPool) {
                                return !e[1].offline && !whitelistAddresses.includes(e[0]) && ((Date.now() / 1000) - e[1].lastBeat) < maxAge;
                            } else {
                                return !e[1].offline && e[1].hr >= minHr && ((Date.now() / 1000) - e[1].lastBeat) < maxAge;
                            }
                        })
                        .map(async e => {
            ...
            return miners.flat().sort((a, b) => {
                if (a.pool === preferedPool && b.pool !== preferedPool) {
                    return -1;
                }
                if (a.pool !== preferedPool && b.pool === preferedPool) {
                    return 1;
